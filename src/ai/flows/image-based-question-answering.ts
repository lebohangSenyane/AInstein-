// A flow that accepts an image and a question, and returns an answer to the question based on the image.
'use server';

/**
 * @fileOverview A flow that accepts an image and a question, and returns an answer to the question based on the image.
 *
 * - imageBasedQuestionAnswering - A function that handles the question answering process.
...
const ImageBasedQuestionAnsweringInputSchema = z.object({
  ...
  question: z.string().describe('The question to be answered about the image.'),
});
export type ImageBasedQuestionAnsweringInput = z.infer<
  typeof ImageBasedQuestionAnsweringInputSchema
>;
...
export type ImageBasedQuestionAnsweringOutput = z.infer<
  typeof ImageBasedQuestionAnsweringOutputSchema
>;
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert AI assistant that answers questions about images.
